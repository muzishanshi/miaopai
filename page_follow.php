					...
					<div class="videoIntr">
						<div class="videoIntrTop">
							...
							<div class="viedoAbout">
								...
								<div class="lists">
								<?php
									$friendLinkRes = $db->query("SELECT * FROM ".DB_PREFIX."link WHERE hide='n' ORDER BY taxis DESC,id DESC LIMIT 0,5");
									if($db->num_rows($friendLinkRes)>0){
										?>
										<div class="link-content"><h3>首页链接</h3>
											<ul>
												<?php
												while ($friendLinkRow = $db->fetch_array($friendLinkRes)) {
													?>
													<li>
														<a href="<?=$friendLinkRow["siteurl"];?>" rel="friend" title="<?=$friendLinkRow["description"];?>" target="_blank">
															<?=$friendLinkRow["sitename"];?>
														</a>
													</li>
													<?php
												}
									...
									$friendLinkRes = $db->query("SELECT * FROM ".DB_PREFIX."link WHERE hide='n' ORDER BY taxis DESC,id DESC LIMIT 5,500");
									if($db->num_rows($friendLinkRes)>0){
										?>
									...
									$friendLinkRes = $db->query("SELECT * FROM ".DB_PREFIX."link WHERE hide='y' ORDER BY taxis DESC,id DESC");
									if($db->num_rows($friendLinkRes)>0){
										?>
